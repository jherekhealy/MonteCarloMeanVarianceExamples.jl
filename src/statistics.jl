export SimulationStatistics, ChanLewis, Kahan, Ling, Klein, Naive, recordValue, lastMeasure,variance, sampleVariance, mean, standardError
...
mutable struct SimulationStatistics{T}
	...
	nextSampleSize::Int
	SimulationStatistics{T}(salgo::StatisticsAlgorithm) where {T} = (reset(salgo); new(salgo, T[],  128 - 1))
...
mutable struct Naive{U} <: StatisticsAlgorithm
	...
	K::U
	useShift::Bool
	Naive{U}(;useShift::Bool=false) where {U} = new(0, zero(U), zero(U), zero(U),useShift)
end
...
mutable struct Klein{T,U} <: StatisticsAlgorithm
	...
	CS2::U
	CCS2::U
	Klein{U}(parent::T) where {T,U} = new{T,U}(parent,zero(U), zero(U),zero(U),zero(U),zero(U),zero(U))
end
...
function updateStatistics(s::Kahan{Ling{T},U}, x) where {U,T}
	...
	y = z-s.Qstar; t = ss.Q + y
    s.Qstar = (t-ss.Q) - y
	ss.Q = t
	z = (x - ss.A)/ss.counter
	y = z - s.Astar; t = ss.A + y
	s.Astar = (t-ss.A) - y
	ss.A = t
end
...
function Base.reset(ss::Klein{U,T}) where {U,T}
	reset(ss.parent)
    ss.S = zero(T); ss.CS = zero(T); ss.CCS = zero(T)
	ss.S2 = zero(T); ss.CS2 = zero(T); ss.CCS2 = zero(T)
end
